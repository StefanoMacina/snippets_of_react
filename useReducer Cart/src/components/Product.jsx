  ...
  return (
    <article className="grid grid-cols-5 mt-5">
      ...
      <div className=" grid items-center ml-4">
        ...
        {countInStock !== 0 ? (
          <h1
            className={`font-semibold text-xs md:text-base lg:text-xl ${
              qty === countInStock && "text-red-600"
            }`}
          >
            in stock: {countInStock}
          </h1>
        ) : (
          <h1 className="font-semibold text-xs md:text-base lg:text-xl text-red-600 text-clip">
            out of stock
          </h1>
        )}
      </div>
      <div className="flex flex-col items-center justify-center">
        <button
          onClick={() => addQtyCountInStock(_id)}
          className="text-green-800"
        >
          <IoIosAddCircle />
        </button>
        {countInStock === 0 ? <p>0</p> : <p>{qty}</p>}
        <button onClick={() => removeQtyInStock(_id)} className="text-red-500">
      ...
      <button
        onClick={() => deleteOneItem(_id)}
        className="flex justify-center items-center text-red-500 lg:text-3xl active:scale-110"
      >
        <MdDelete />
      </button>
    </article>
